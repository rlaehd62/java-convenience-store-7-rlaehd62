...
import store.config.FixedLocalDateTime;
...
class ApplicationTest extends NsTest {
    ...
    void 기간에_해당하지_않는_프로모션_적용() {
        DateTimeUtil.getInstance().resetFixedMode();
        assertNowTest(() -> {
            run("[감자칩-2]", "N", "N");
    ...
    void 프로모션_종료일_이후_구매() {
        assertNowTest(() -> {
            run("[탄산수-3]", "N", "N");
            assertThat(output().replaceAll("\\s", "")).contains("내실돈2,400"); // 탄산2+1 프로모션이 만료되어 적용되지 않음
